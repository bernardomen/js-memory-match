...
MemoryJavascript.prototype.changeSource = function(id){
	...
	if(this.comparator.length > 0 && this.comparatorContainsValue(id)){
		document.getElementById(this.comparator[this.comparator.length -1].index).src="https://i2.wp.com/codebits.eu/logos/defaultavatar.jpg";
		this.comparator.pop();
		this.allowedMoves = 2;
		return;
	}
	...
	document.getElementById(id).src = this.gameDataArray[numId].img;
	...
	if(this.allowedMoves === 0){
		...
		if(this.comparator[this.comparator.length -1].src !== this.comparator[this.comparator.length -2].src){
			...
			if(this.availablePairs === 0){
				...
				link.href = "https://twitter.com/intent/tweet/?text=Memory JavaScript FTW in: "+ time;
				link.innerHTML = "Share your score on twitter";
				link.id = "twitterLink";
...
MemoryJavascript.prototype.resetPair = function(){
	document.getElementById(this.comparator[this.comparator.length -1].index).src="https://i2.wp.com/codebits.eu/logos/defaultavatar.jpg";
	document.getElementById(this.comparator[this.comparator.length -2].index).src="https://i2.wp.com/codebits.eu/logos/defaultavatar.jpg";
	this.comparator.pop();
	this.comparator.pop();
};
...
MemoryJavascript.prototype.updateTimer = function(){
	...
	time++;
	document.getElementById("timerP").innerHTML = time + " seconds";
};
